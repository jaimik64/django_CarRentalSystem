    ...
    for a in area:
        ...
        for car in vehicles:
            if car.is_available == True:
                vehicle_dictionary = {'name':car.car_name, 'color':car.color, 'id':car.id, 'pincode':car.area.pincode, 'capacity':car.capacity, 'description':car.description, 'image': car.img.url, 'is_available': car.is_available}
...
def update_order(request):
    messages.info(request, 'Your order will be cancelled and you need to place the order again.')
...
@login_required
def update(request):
    return render(request,'customer/updateProfile.html')
