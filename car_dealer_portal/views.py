...
def add_vehicle(request):
    ...
    try:
        ...
        area = None
        if area is not None:
            car = Vehicles(car_name=car_name, color=color, dealer=cd, area = area, description = description, capacity=capacity, img = img)
        else:
            area = Area(city = city, pincode = pincode)
            area.save()
            area = Area.objects.get(city = city, pincode = pincode)
...
def update(request):
    ...
    obj = get_object_or_404(User, id = veh_id)
    ...
    return render(request, "car_dealer/update.html", context)
